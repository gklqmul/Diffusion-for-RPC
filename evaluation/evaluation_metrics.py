...
def calculate_permutation_aware_mpje(recons: torch.Tensor, x: torch.Tensor) -> float:
    """
    计算基于最佳匹配的平均每关节位置误差 (MPJE)。

    该函数会为批次中的每个样本独立使用匈牙利算法寻找 pred 和 GT 之间的最佳匹配，
    然后再计算平均误差。

    Args:
        recons (torch.Tensor): 重建的骨骼点张量, 形状 (B, N_joints, 3)。
        x (torch.Tensor): 原始的骨骼点张量, 形状 (B, N_joints, 3)。
    
    Returns:
        float: 平均 MPJE。
    """
    assert recons.shape == x.shape
    B, N_joints, _ = recons.shape

    # 将 PyTorch Tensor 移动到 CPU 并转换为 NumPy 数组
    ...
    # 遍历批次中的每个样本 (帧)
    for i in range(B):
        pred_joints = recons_np[i]  # (N_joints, 3)
        gt_joints = x_np[i]         # (N_joints, 3)

        # 1. 计算距离矩阵 D (N_joints x N_joints)
        # D[i, j] 是 pred_joints[i] 到 gt_joints[j] 的距离
        
        # 使用 NumPy 广播机制计算所有点对的欧氏距离
        diff = pred_joints[:, np.newaxis, :] - gt_joints[np.newaxis, :, :] # (N, N, 3)
        D = np.linalg.norm(diff, axis=-1) # (N, N) 距离矩阵

        # 2. 找到最佳对应（匈牙利算法）
        # 目标是最小化总距离
        row_ind, col_ind = linear_sum_assignment(D)

        # row_ind: 预测点的索引 (0, 1, 2, ...)
        # col_ind: 真实点的最佳匹配索引 (新的顺序)
        
        # 3. 提取最佳匹配的距离并计算 MPJE
        matched_distances = D[row_ind, col_ind]
        
        # 计算当前样本的 MPJE (N_joints 个距离的平均值)
        sample_mpje = np.mean(matched_distances)
        batch_mpjes.append(sample_mpje)

    # 计算整个批次的平均 MPJE
    avg_mpje = np.mean(batch_mpjes)

    return float(avg_mpje)
